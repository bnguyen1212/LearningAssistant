...
class LearningAssistantInterface:
    ...
    def detect_note_request(self, user_message: str) -> bool:
        """
        Detect if user wants to save/take notes from conversation
        """
        note_triggers = [
        ...
        detected = any(trigger in message_lower for trigger in note_triggers)
        
        if detected:
            print("📝 Note saving request detected!")
    ...
    def process_user_input(self, user_input: str) -> bool:
        ...
        try:
            ...
            # Detect note request before passing to workflow
            note_requested = self.detect_note_request(user_input)
            self.current_state["note_request_detected"] = note_requested
